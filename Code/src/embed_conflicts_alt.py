"""Embed all conflict (non-CONTROL) Wikipedia leads with an alt embedder.

Sister script to embed_controls_alt.py. Reads non-CONTROL_ raw files
from data/.../raw/ and writes per-embedder output to
data/.../processed_leads_alt/<embedder>/<slug>_<lang>.json.

Used for the exp_015 Wikipedia source-side cross-embedder check, which
needs ALL conflict articles (~1,224) re-embedded under each alt
embedder for an apples-to-apples comparison vs the OpenAI baseline.
"""
...
def main() -> None:
    ...
    files = sorted(p for p in RAW_DIR.iterdir()
                   ...
                   and p.name.endswith("_raw.json"))
    print(f"Embedding {len(files)} conflict leads")
    ...
    for src in files:
        ...
        # Parse "<topic>_<lang>_raw.json" → topic + lang
        stem = src.name.removesuffix("_raw.json")
        parts = stem.split("_")
        lang = parts[-1]
        topic = " ".join(parts[:-1])
